	...
	Properties{
		...
		SubShader{
			...
			Pass {
				...
				uniform float _MainPolarCoord;
				uniform float _AlphaMask;
				uniform float _MainAlphaCut;
				uniform float4 _MainSpeed;
				uniform float _Mainoffset;
				uniform half _CVS_UV;
				uniform half _DissolveTex;
				uniform sampler2D _MaskTex; uniform float4 _MaskTex_ST;
				uniform float _MaskPolarCoord;
				uniform float4 _MaskSpeed;
				uniform float _MaskAlphaCut;
				uniform float _Texture02;
				uniform float _MainMask;
				uniform half _CVS_UV2;
				uniform float4 _Tex02Color;
				...
				uniform float _FresnelChanel;
				uniform float _VertexOffsetInt;
				...
				VertexOutput vert(VertexInput v) {
					...
					o.normalDir = TransformObjectToWorldNormal(v.normal.xyz, true);


					//顶点偏移
					#if defined(_VERTEXOFFSET_ON) && defined(_NOISE_ON)
					float2 VOPolarUVRamp = o.uv0 * 2.0 + -1.0;
					float2 VOPolarCoord = float2(((atan2(VOPolarUVRamp.r,VOPolarUVRamp.g) / UNITY_TWO_PI) + 0.5),length(VOPolarUVRamp));
					float2 VOSpeed = _Time.g * _NoiseSpeed.xy;
					float4 _VO_var = tex2Dlod(_NoiseTex,float4(TRANSFORM_TEX((VOPolarCoord + VOSpeed), _NoiseTex),0.0,0));
				...
				float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
					...
					////// Emissive:
									...
									float2 PolarCoord = float2(((atan2(PolarUVRamp.r,PolarUVRamp.g) / UNITY_TWO_PI) + 0.5),length(PolarUVRamp));
									#endif


									//溶解?
									#if defined(_DISSOLVETOGGLE_ON)
									float SmoothStepMax = (_DissolveStep * 0.5 + 0.5);
									float SmoothStepMin = (1.0 - SmoothStepMax);
									#endif
									...
									float2 MaskUV = lerp(float2(_Time.g * _MaskSpeed.xy), i.uv1.xy, _CVS_UV2) + MaskUVPolar;
									float4 _MaskTex_var = tex2D(_MaskTex,TRANSFORM_TEX(MaskUV , _MaskTex));
									float4 MaskColor = float4(_MaskTex_var.rgb,saturate(_MaskTex_var.a - _MaskAlphaCut)) * _Intensity.y * _Tex02Color * i.vertexColor.a;
									#endif
									...
									float4 _NoiseTex_var = tex2D(_NoiseTex,TRANSFORM_TEX((NoiseUVPolar + float2(_Time.g * _NoiseSpeed.xy)), _NoiseTex));
									float2 NoiseInt = (_NoiseTex_var.rg * _NoiseInt);

									//遮罩==>噪波
									#if defined(_MASK_ON)
									float NoiseMask = _NoiseMask == 1 ? MaskColor.a : 1;
									NoiseInt *= NoiseMask;
									#endif
									...
									float2 MainUV = lerp(float2(_Time.g * _MainSpeed.xy), i.uv1.xy, _CVS_UV) + MainUVPolar;
									float4 _MainTex_var = tex2D(_MainTex,TRANSFORM_TEX(MainUV , _MainTex));
									float4 MainColor = _Intensity.x * _TintColor * i.vertexColor;

									//遮罩==>主贴图
									#if defined(_MASK_ON)                       
									float MainMask = _AlphaMask == 1 ? MaskColor.a : 1;
									MainColor *= MainMask;
									#endif
									...
									float4 Mainoffset_R = tex2D(_MainTex,TRANSFORM_TEX((MainUV + _Mainoffset.xx), _MainTex));
									float4 Mainoffset_B = tex2D(_MainTex,TRANSFORM_TEX((MainUV - _Mainoffset.xx), _MainTex));
									_MainTex_var = float4(Mainoffset_R.r, _MainTex_var.g, Mainoffset_B.b, _MainTex_var.a);
									#endif

								  float4 finalMain = float4(_MainTex_var.rgb,saturate(_MainTex_var.a - _MainAlphaCut)) * MainColor;

								  //遮罩==>纹理2
								  #if defined(_MASK_ON)
								  float3 Tex02 = _Texture02 == 1 ? MaskColor.rgb : 0;
								  float Tex02Alpha = _MainMask == 1 ? 0 : MaskColor.a;
								  finalMain = float4((finalMain.rgb + Tex02 * MaskColor.a),(finalMain.a + Tex02Alpha));
								  #endif

								  //噪波==>全局遮罩
								  #if defined(_NOISE_ON)
								  float Mask02 = _Mask02 == 1 ? _NoiseTex_var.a : 1;
								  finalMain *= Mask02;
								  #endif
								  ...
								  //溶解==>主效果
									...
									//溶解纹理
									#if defined(_NOISE_ON)
									float MainDissolveTex = _DissolveTex == 1 ? _NoiseTex_var.r : _MainTex_var.a;
									#else
									float MainDissolveTex = _MainTex_var.a;
									#endif
								  ...
								  if(_DissolveColType == 1)
								  {
								    ...
								    half3 explsionCol = lerp(_EdgeColor1,_EdgeColor2,t);
								    //half3 explsionCol = step(0.5,t) < 0 ? _EdgeColor1 : _EdgeColor2;
								    finalMain = float4(lerp(finalMain,explsionCol,t * step(0.0001,i.uv1.b)),finalMain.a * (Dissolve + EdgeAlpha));
								  }
								  else
								  {
								  ...
								  float FresnelAlpha = pow(1.0 - max(0,dot(normalDirection, viewDirection)),lerp(_Exp,i.uv1.a,_CVS_W));
								  float4 finalFres = i.vertexColor * _FresnelColor * FresnelAlpha * _Intensity.w;

								  //遮罩==>菲涅尔
								  #if defined(_MASK_ON)
								  float FresMask = _AlphaMask == 1 ? MaskColor.a : 1;
								  finalFres *= FresMask;
								  #endif

								  //溶解==>菲涅尔
								  #if defined(_DISSOLVETOGGLE_ON)
								  float FresDissolveTex = 0;
								...
								EdgeColor = _EdgeColor1 * DissolveEdge * _Intensity.z;
								EdgeAlpha = DissolveEdge * i.vertexColor.a * _Intensity.z;
								finalFres = float4(finalFres.rgb * Dissolve + EdgeColor,finalFres.a * (Dissolve + EdgeAlpha));
								#endif
							  #endif

							  float4 finalEffect = 0;
							  ...
							  finalEffect = float4(finalMain.rgb + finalFres.rgb,finalMain.a + finalFres.a);
