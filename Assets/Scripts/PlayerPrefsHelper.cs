...
public static class PlayerPrefsHelper
{
    ...
    static string encryptKey = "FLFrame";
    static string encryptVector = "enFramework";
    ...
    {
        prefKey = $"{prefKey}_{projectPath}";
        string encryptedValue = AESEncrypt.Encrypt(value, encryptKey, encryptVector);
        Debug.Log($"±£´ækey:[{prefKey}],ÄÚÈÝ:[{value}]");
    ...
    {
        prefKey = $"{prefKey}_{projectPath}";
        string encryptedStrValue = AESEncrypt.Encrypt(value.ToString(), encryptKey, encryptVector);
    ...
    {
        prefKey = $"{prefKey}_{projectPath}";
        string encryptedDateTime = AESEncrypt.Encrypt(value.ToString(), encryptKey, encryptVector);
    ...
    public static void SetCustom<T>(string prefKey, T value)
    {
        ...
        string encryptedJson = AESEncrypt.Encrypt(json, encryptKey, encryptVector);
    ...
    public static string GetString(string prefKey, string defaultValue = "")
    {
        ...
        if (!PlayerPrefs.HasKey(prefKey) || encryptedValue == "")
            return defaultValue;
        return AESEncrypt.Decrypt(encryptedValue, encryptKey, encryptVector);
    }
    ...
    public static T GetEnum<T>(string prefKey, T defaultValue) where T : Enum
    {
        ...
        string strValue = AESEncrypt.Decrypt(encryptedStrValue, encryptKey, encryptVector);
    ...
    public static DateTime? GetDateTime(string prefKey, DateTime? defaultValue = null)
    {
        ...
        string dateTimeString = AESEncrypt.Decrypt(encryptedDateTimeString, encryptKey, encryptVector);
